...
function normalizeDivisionAlias_(valueRaw) {
  ...
  const tokens = value.split(/[^a-z�-�0-9]+/).filter(Boolean);
  const hasToken = token => tokens.indexOf(token) !== -1;

  if (hasToken('��') || hasToken('map')) return '��';
  if (hasToken('���') || hasToken('dms')) return '���';
  if (hasToken('�����������') || hasToken('laboratory') || tokens.some(token => token.indexOf('�����') === 0)) return '�����������';
  if (hasToken('�����') || hasToken('metro')) return '�����';
  if (hasToken('��') || hasToken('constructioncontrol')) return '��';
...
function getSourceDivisionNorm_(sourceRaw) {
  ...
  if (source === 'dms') return '���';
  if (source === 'laboratory') return '�����������';
  if (source === 'metro') return '�����';
  if (source === 'constructioncontrol') return '��';
  return '';
}
